...
> **はじめて遊ぶ方へ** — この文書はキーと画面の「リファレンス」です。
> 「何をすればいいのか」から知りたい場合は **[PLAYGUIDE.md](PLAYGUIDE.md)（遊びかたガイド）** を先に読んでください。
...
### スーパークルーズ中
- スロットルで速度が指数的に変化（30 km/s → 2001c）。スロットルは 0〜100% のみ有効（マイナスは最低速に丸められます）
- 天体に接近すると**重力井戸**が速度上限を強制減衰（EDと同様の減速カーテン）。効きぐあいは HUD の GRAV WELL バーで確認できます
- 速度表示: `km/s` → `c` → `kc` 自動切替
...
| GRAV WELL | FAランプ下 | **SC中のみ**。重力井戸の深さ 0-100%。`CLEAR`(水色) / `FIELD`(橙) / `BRAKING`(赤・点滅)。白い縦線が BRAKING の閾値(82%) |
| FUEL/HEAT | 左下 | 燃料残量・熱（>80%で赤点滅）。※どちらも表示のみで、尽きても影響はありません |
